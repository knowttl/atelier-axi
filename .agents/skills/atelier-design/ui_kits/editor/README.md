# Atelier Editor — UI kit

A clickable design reference for the **Atelier Editor chrome** that the `atelier-axi` CLI launches in a browser.
...
Everything is fake — there's no `atelier-axi` server backing it. The reply text is canned. The DOM snapshot is not transmitted anywhere.
...
- DOM snapshot serialization. `atelier.snapshot()` would produce something like the tree in `src/artifact-sdk.js`; we don't.
- File-path identity, session store, long-polling.

Read `src/server.js`, `src/chrome-client.js`, `src/chrome.css`, and `src/artifact-sdk.js` in the atelier-axi repo for the canonical behaviour.
