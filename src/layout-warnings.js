...
// from the Atelier top bar, and only an explicit "Queue selected fixes" turns them into an
...
export function layoutWarningPromptPayload(warnings) {
  ...
  const prompt =
    ...
    "A queued layout issue is a repair request, not a resolved issue: Atelier only marks it resolved after a newer artifact load and a complete diagnostic pass for the same viewport no longer detects it.";
  const target = {
...
export function resolveDiagnosticViewportClasses(env = process.env) {
  const raw = String(env.ATELIER_AXI_DIAGNOSTIC_VIEWPORTS || "").trim();
