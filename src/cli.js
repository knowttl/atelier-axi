...
export const POLL_WAKE_PATH_RULES = Object.freeze([
  ...
  "Run at most one poll per artifact: a session delivers each batch of feedback to a single poll, so starting a second one retires the first with a POLL_SUPERSEDED error rather than splitting the user's feedback between them.",
]);
...
export function createPollOutput({ file, response, full = false, agent = "generic" }) {
  ...
  // A session delivers its feedback to exactly one poll. Being superseded means a newer poll owns
  // the session now, so this one returns nothing - loudly, because the alternative is two pollers
  // splitting one batch between them with neither the user nor either agent told.
  if (response.status === "superseded") {
    throw new AxiError("Another `atelier-axi poll` took over this Atelier Editor session", "POLL_SUPERSEDED", [
      "No feedback was delivered to this poll and none was lost - the newer poll receives it",
      "Run only one poll per artifact: stop the leftover poll, or let this one exit and keep the newer one",
