...
const warningAckStorageKey = "atelier-axi:warning-ack:" + key;
...
function revealWarning(warning) {
  postToFrame({ type: "atelier:revealElement", selector: warning.selector });
}
...
async function replaceArtifactFrame() {
  ...
  if (!artifactSrc) {
    ...
    frame.src = currentSrc + (currentSrc.includes("?") ? "&" : "?") + "atelier_reload=" + Date.now();
    return true;
  }
  const requestSequence = ++artifactLoadRequestSequence;
  const requestId = `atelier-load-${Date.now().toString(36)}-${requestSequence}-${Math.random().toString(36).slice(2)}`;
...
function showWhiteboardOverlay(index) {
  ...
  whiteboardOverlay.hidden = false;
  postToFrame({ type: "atelier:suspendWhiteboard", diagramIndex: index });
...
function finishWhiteboardClose(index) {
  ...
  overlayChannelId = "";
  inlineWhiteboardChannels.delete(index);
  if (!ended) postToFrame({ type: "atelier:resumeWhiteboard", diagramIndex: index });
}
...
function beginWhiteboardTeardown(index, placement, onComplete) {
  ...
  const message = { type: "atelier-whiteboard:prepareTeardown", flushId };
...
function beginWhiteboardFlush(index, placement) {
  ...
  postToWhiteboard(index, placement, { type: "atelier-whiteboard:flush", flushId });
  return promise;
}
...
function handleWhiteboardSave(index, message, mode) {
  ...
  saveWhiteboardScene(index, message).then(
    () => {
      if (flushId) postToWhiteboard(index, mode, { type: "atelier-whiteboard:saveResult", flushId, ok: true });
    },
    (error) => {
      if (flushId) {
        postToWhiteboard(index, mode, {
          type: "atelier-whiteboard:saveResult",
          flushId,
          ok: false,
...
async function queueWhiteboardFeedback(index, message, mode) {
  ...
  try {
    ...
    postToWhiteboard(index, mode, { type: "atelier-whiteboard:queueResult", ok: true });
    if (mode === "overlay") closeWhiteboard();
  } catch (error) {
    postToWhiteboard(index, mode, {
      type: "atelier-whiteboard:queueResult",
      ok: false,
      error: error instanceof Error ? error.message : String(error),
    });
  }
  ...
  try {
    ...
    if (nextHash !== record.sourceHash) {
      ...
      postToWhiteboardOverlay({
        type: "atelier-whiteboard:sourceChanged",
        source: record.source,
        sourceHash: record.sourceHash,
      });
    }
...
function handleAuthenticatedWhiteboardMessage(index, message, mode) {
  if (message.type === "atelier-whiteboard:save") handleWhiteboardSave(index, message, mode);
  if (message.type === "atelier-whiteboard:queueFeedback") queueWhiteboardFeedback(index, message, mode);
  if (message.type === "atelier-whiteboard:maximize" && mode === "inline") openWhiteboardOverlay(index);
  if (message.type === "atelier-whiteboard:close" && mode === "overlay") closeWhiteboard();
  if (message.type === "atelier-whiteboard:teardownReady") finishWhiteboardTeardown(index, message, mode);
  if (message.type === "atelier-whiteboard:teardownFailed") failWhiteboardTeardown(index, message, mode);
  if (message.type === "atelier-whiteboard:flushComplete") finishWhiteboardFlush(index, message, mode);
}
...
function handleInlineWhiteboardMessage(event, message) {
  ...
  if (message.type === "atelier-whiteboard:ready") {
...
window.addEventListener("message", (event) => {
  ...
  clearTimeout(artifactSilenceTimer);
  if (msg.type === "atelier:layoutDiagnostics") {
...
frame.addEventListener("load", () => {
  ...
  if (lastReviewState) postToFrame({ type: "atelier:restoreReviewState", state: lastReviewState });
  if (overlayIndex !== null) {
    inlineWhiteboardChannels.delete(overlayIndex);
    postToFrame({ type: "atelier:suspendWhiteboard", diagramIndex: overlayIndex });
  }
});
