  ...
  window.top.postMessage(
    {
      ...
      ...(state.channelId
        ...
        : {
            channelToken: String(/** @type {any} */ (window).__atelierWhiteboardChannelToken || ""),
...
function buildShell(theme, mode) {
  document.body.dataset.atelierWhiteboardTheme = theme;
  document.body.dataset.atelierWhiteboardMode = mode;
  ...
  if (mode === "inline") {
    ...
    fullscreenButton.onclick = () => post({ type: "atelier-whiteboard:maximize", diagramIndex: state.diagramIndex });
...
function prepareTeardown(message) {
  ...
  if (!postSave(flushId)) {
    ...
    post({ type: "atelier-whiteboard:teardownReady", flushId });
  }
}
...
function flushSaveNow(message) {
  ...
  if (!postSave(flushId)) {
    ...
    post({ type: "atelier-whiteboard:flushComplete", flushId, ok: true });
  }
}
...
function handleSaveResult(message) {
  ...
  if (flushId === state.teardownFlushId) {
    ...
    post({ type: "atelier-whiteboard:teardownFailed", flushId, error });
    return;
  }
  if (state.flushIds.delete(flushId)) {
    post({ type: "atelier-whiteboard:flushComplete", flushId, ok: Boolean(message.ok) });
  }
}
...
function main() {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (msg.type === "atelier-whiteboard:init" && !initialized && typeof msg.channelId === "string" && msg.channelId) {
      initialized = true;
    ...
    if (msg.type === "atelier-whiteboard:sourceChanged") handleSourceChanged(msg);
    if (msg.type === "atelier-whiteboard:prepareTeardown") prepareTeardown(msg);
    if (msg.type === "atelier-whiteboard:flush") flushSaveNow(msg);
    if (msg.type === "atelier-whiteboard:saveResult") handleSaveResult(msg);
    if (msg.type === "atelier-whiteboard:queueResult") {
      resetQueueButton();
      if (msg.ok) {
  ...
  post({ type: "atelier-whiteboard:ready" });
}
