...
export class SessionStore {
  constructor(file) {
    this.file = file;
    // One queue serializes every state.json read-modify-write and the server's
  ...
  async upsertSession(file, url) {
    ...
    // section.
    const absolute = await canonicalFile(file);
    return this.runExclusive(() => this.#upsertSessionLocked(absolute, url));
  }
  ...
  async queuePrompts(key, payload, options = {}) {
    ...
    return this.runExclusive(() => this.#queuePromptsLocked(key, payload, options));
  }
  ...
  // chain alive whether it resolves or rejects so one failed op can't wedge the store, and so a
  // rejected predecessor never skips its successor.
  /**
   * @template T
   * @param {() => Promise<T>} operation
   * @returns {Promise<T>}
  ...
  runExclusive(operation) {
    const result = this.stateOperationQueue.then(operation, operation);
    this.stateOperationQueue = result.then(noop, noop);
    return result;
  }
  ...
  // This pure read must not take the queue itself because the server calls it from
  // inside runExclusive to keep the snapshot atomic with the subsequent disk delete.
