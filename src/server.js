...
// next to the served bundle; source runs (node bin/atelier-axi.js) fall back to
...
export async function serve({
  ...
  // chromes - it never emits "feedback", so it can never make `atelier-axi poll` return and can
    ...
    try {
      ...
      if (!beforeRead.valid) {
        res
          ...
          .type("html")
          .send(
            "<!doctype html><title>Artifact load expired</title><p>This artifact load is no longer current. Reload Atelier to continue.</p>",
          );
        return;
      }
      ...
      res.type("html").send(injectAtelierSdk(html, key, verified.artifact_revision, verified.artifact_load_token));
    } catch (error) {
      next(error);
    }
  });
...
// A lone "*" in ATELIER_AXI_ALLOWED_HOSTS is an explicit opt-out of the Host
...
// ATELIER_AXI_ALLOWED_HOSTS. This is an AND check: a client-spoofed forwarded host
...
export function createChromeHtml(
  session,
  {
    ...
    faviconTag = ATELIER_DEFAULT_FAVICON,
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Atelier Whiteboard</title>
<link rel="stylesheet" href="/whiteboard-assets/whiteboard.css">
</head>
<body>
<script>window.__atelierWhiteboardChannelToken=${JSON.stringify(channelToken)};</script>
