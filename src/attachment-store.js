...
// Limits are configurable in the ATELIER_AXI_* style, mirroring the idle-timeout
...
export function resolveAttachmentConfig(env = process.env) {
  const maxDiskBytes = diskCapEnv(env.ATELIER_AXI_MAX_ATTACHMENT_DISK_MB);
  return {
    maxBytes: positiveIntEnv(env.ATELIER_AXI_MAX_ATTACHMENT_BYTES, DEFAULT_MAX_ATTACHMENT_BYTES),
    maxPerPrompt: positiveIntEnv(env.ATELIER_AXI_MAX_ATTACHMENTS_PER_PROMPT, DEFAULT_MAX_ATTACHMENTS_PER_PROMPT),
    maxPromptBytes: positiveIntEnv(env.ATELIER_AXI_MAX_PROMPT_ATTACHMENT_BYTES, DEFAULT_MAX_PROMPT_ATTACHMENT_BYTES),
    ttlMs: durationEnv(env.ATELIER_AXI_ATTACHMENT_TTL_MS, DEFAULT_ATTACHMENT_TTL_MS),
    maxDiskBytes,
