...
function tempDir(prefix = "atelier-plugin-") {
...
const symlinkSupport = (() => {
  const probe = mkdtempSync(path.join(os.tmpdir(), "atelier-symlink-probe-"));
  try {
...
test("normalizeRepositoryUrl converts npm git URLs to plain https", () => {
  assert.equal(
    normalizeRepositoryUrl({ url: "git+https://github.com/kunchenguid/atelier-axi.git" }),
    "https://github.com/kunchenguid/atelier-axi",
  );
...
test("the package root is itself a discoverable Agent Plugin", async () => {
  ...
  assert.deepEqual(
    ...
    ["atelier"],
    "exactly the atelier skill is discovered",
  );

  const skill = await readFile(path.join(root, "skills", "atelier", "SKILL.md"), "utf8");
  assert.deepEqual(validateSkillMarkdown(skill, { directoryName: "atelier" }).errors, []);
});

test("the plugin declares no MCP servers", async () => {
  // atelier-axi's agent surface is the CLI itself; an mcp.json would add a second contract.
...
test("VS Code registration adds the plugin root and is idempotent", () => {
  const [first, changedFirst] = computeVsCodePluginLocationsUpdate({}, "/pkg/atelier-axi", "atelier-axi");
  assert.equal(changedFirst, true);
  assert.deepEqual(first["chat.pluginLocations"], { "/pkg/atelier-axi": true });

  const [second, changedSecond] = computeVsCodePluginLocationsUpdate(first, "/pkg/atelier-axi", "atelier-axi");
...
test("VS Code registration preserves unrelated settings and other plugins", () => {
  ...
  const [updated] = computeVsCodePluginLocationsUpdate(settings, "/pkg/atelier-axi", "atelier-axi");
  ...
  assert.equal(updated["chat.pluginLocations"]["/pkg/atelier-axi"], true);
...
test("VS Code registration repairs a relocated install without dropping foreign entries", () => {
  const dir = tempDir();
  const stale = writePlugin(path.join(dir, "old", "atelier-axi"), "atelier-axi");
  const foreign = writePlugin(path.join(dir, "other-plugin"), "other-plugin");
  const current = writePlugin(path.join(dir, "new", "atelier-axi"), "atelier-axi");

  const settings = { "chat.pluginLocations": { [stale]: true, [foreign]: true } };
  const [updated, changed] = computeVsCodePluginLocationsUpdate(settings, current, "atelier-axi");

  assert.equal(changed, true);
  assert.equal(updated["chat.pluginLocations"][stale], undefined, "the previous atelier-axi location is dropped");
...
test("a removed install directory is only treated as stale when it was ours", () => {
  const dir = tempDir();
  assert.equal(isStalePluginLocation(path.join(dir, "gone", "atelier-axi"), "atelier-axi"), true);
  assert.equal(isStalePluginLocation(path.join(dir, "gone", "someone-else"), "atelier-axi"), false);
});
...
test("Cursor registration links, no-ops, and repairs the local plugin slot", { skip: symlinkSupport.skip }, () => {
  ...
  const pluginRoot = writePlugin(path.join(dir, "pkg", "atelier-axi"), "atelier-axi");

  const linked = linkCursorLocalPlugin(localPlugins, pluginRoot, "atelier-axi");
  assert.equal(linked.status, "linked");
  assert.equal(path.resolve(readlinkSync(linked.target)), pluginRoot);

  assert.equal(linkCursorLocalPlugin(localPlugins, pluginRoot, "atelier-axi").status, "current");

  const moved = writePlugin(path.join(dir, "pkg2", "atelier-axi"), "atelier-axi");
  // Exercise Windows' move-aside replacement path even when this suite runs elsewhere.
  const repaired = linkCursorLocalPlugin(localPlugins, moved, "atelier-axi", { platform: "win32" });
...
test("Cursor registration refuses to clobber a real directory in the slot", () => {
  const dir = tempDir();
  const localPlugins = path.join(dir, "local");
  const occupied = path.join(localPlugins, "atelier-axi");
  mkdirSync(occupied, { recursive: true });
  writeFileSync(path.join(occupied, "keep.txt"), "user content");

  const result = linkCursorLocalPlugin(localPlugins, path.join(dir, "pkg"), "atelier-axi");
...
test("Cursor registration replaces a dangling symlink", { skip: symlinkSupport.skip }, () => {
  ...
  symlinkSync(path.join(dir, "vanished"), path.join(localPlugins, "atelier-axi"));
  const pluginRoot = writePlugin(path.join(dir, "pkg", "atelier-axi"), "atelier-axi");

  const result = linkCursorLocalPlugin(localPlugins, pluginRoot, "atelier-axi");
...
test("Cursor registration preserves the old link when replacement fails", { skip: symlinkSupport.skip }, async () => {
  const dir = tempDir();
  const localPlugins = path.join(dir, "local");
  const original = writePlugin(path.join(dir, "old", "atelier-axi"), "atelier-axi");
  const replacement = writePlugin(path.join(dir, "new", "atelier-axi"), "atelier-axi");
  mkdirSync(localPlugins, { recursive: true });
  const target = path.join(localPlugins, "atelier-axi");
  symlinkSync(original, target);

  const result = linkCursorLocalPlugin(localPlugins, replacement, "atelier-axi", {
    renameSync: () => {
  ...
  assert.deepEqual(await readdir(localPlugins), ["atelier-axi"]);
});
...
test("Cursor registration reports rather than throws when links cannot be created", () => {
  ...
  const result = linkCursorLocalPlugin(path.join(dir, "local"), path.join(dir, "pkg"), "atelier-axi", {
    symlinkSync: denied,
  });
...
test("plugin client launch preserves Windows batch arguments", { skip: process.platform !== "win32" }, async () => {
  const dir = tempDir("atelier plugin client ");
  ...
  const pluginRoot = path.join(dir, "Jane Doe & team", "atelier-axi");
