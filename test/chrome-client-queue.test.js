...
async function createChromeHarness({
  ...
  if (storedQueue) storage.set(`atelier-axi:queued:${sessionData.key}`, JSON.stringify(storedQueue));
  const postedToFrame = [];
  ...
  return {
    ...
    sendSnapshot(
      ...
      const handlers = windowListeners.get("message") || [];
      assert.ok(handlers.length > 0, "chrome-client registered a message handler");
      // Snapshot replies cross the same load-token gate as any other artifact message, so stamp
      // the current load token exactly as the real SDK's postArtifactMessage would.
      const data = {
      ...
      if (artifactSrc) data.artifact_load_token = frameLoadToken();
...
test("chrome mediates attachment uploads: rate + cumulative-byte ceiling (confused-deputy guard)", async () => {
  ...
  const invalidResult = chrome.postedToFrame.find(
    (m) => m.type === "atelier:attachmentResult" && m.localId === "invalid",
  );
  ...
  const quotaResult = chrome.postedToFrame.find((m) => m.type === "atelier:attachmentResult" && m.localId === "big");
...
test("chrome only mediates uploads carrying the current artifact load token", async () => {
  ...
  assert.equal(
    chrome.postedToFrame.some((m) => m.type === "atelier:attachmentResult" && m.localId === "no-token"),
    false,
  ...
  const result = chrome.postedToFrame.find((m) => m.type === "atelier:attachmentResult" && m.localId === "with-token");
...
test("queued annotation prompts still deliver while a composer chip uploads", async () => {
  ...
  assert.ok(chrome.postedToFrame.some((m) => m.type === "atelier:requestSnapshot"));
  chrome.sendSnapshot("uid=1 body");
  await flushPromises();
  await flushPromises();
...
test("a failed diagnostic pass reports its incompleteness rather than an empty result", async () => {
  ...
  chrome.sendFrameMessage({ type: "atelier:layoutDiagnostics", complete: false, viewport_width: 720, findings: [] });
  await flushPromises();
...
test("a stale queued layout prompt remains available for user re-decision", async () => {
  ...
  row.children[0].dispatch("change");
  await chrome.element("warningsQueueButton").onclick();
  // Queueing a fix only enqueues the prompt. An unsolicited snapshot must not drive a submit
  // here - a snapshot reply has to answer a request this chrome actually made - so send
  // explicitly and then answer that request.
...
test("Reveal asks the artifact iframe to highlight the affected element", async () => {
  ...
  assert.equal(revealMessage.type, "atelier:revealElement");
...
test("the layout gate reveals after a completed pass with no findings", async () => {
  ...
  chrome.sendFrameMessage({ type: "atelier:layoutDiagnostics", complete: true, viewport_width: 720, findings: [] });
  await flushPromises();
...
test("a current load token accepts artifact messages before the frame load event", async () => {
  ...
  chrome.sendFrameMessage({
    ...
    type: "atelier:artifactAssetFailure",
...
test("a pre-load diagnostic silences the probe even while its response is delayed", async () => {
  ...
  chrome.eventSource().listeners.get("reload")();
  await flushPromises();
  chrome.sendFrameMessage({ type: "atelier:layoutDiagnostics", complete: true, findings: [] });
  await flushPromises();
  chrome.frame.dispatch("load");
  chrome.runTimers(8000);
...
test("stale artifact messages are ignored until the current frame load", async () => {
  ...
  chrome.sendFrameMessage({ artifact_load_token: oldToken, type: "atelier:scroll", x: 8, y: 44 });
  chrome.sendFrameMessage({
    artifact_load_token: oldToken,
    type: "atelier:artifactAssetFailure",
    detail: "stale asset",
  ...
  chrome.frame.dispatch("load");
  assert.equal(
    chrome.postedToFrame.some((message) => message.type === "atelier:restoreReviewState"),
    false,
  );
  const restoredScroll = chrome.postedToFrame.filter((message) => message.type === "atelier:restoreScroll").at(-1);
  assert.equal(restoredScroll.x, 0);
  assert.equal(restoredScroll.y, 0);

  chrome.sendFrameMessage({ type: "atelier:artifactAssetFailure", detail: "current asset" });
  await flushPromises();
  assert.equal(posts.filter((post) => post.url === "/api/abc/artifact-failures").length, 1);
});
...
test("a delayed diagnostic response does not delay silencing the artifact probe", async () => {
  ...
  chrome.sendFrameMessage({ type: "atelier:layoutDiagnostics", complete: true, viewport_width: 1440, findings: [] });
  await flushPromises();
  chrome.runTimers(8000);
  await flushPromises();
...
test("artifact relays cannot invoke whiteboard persistence", async () => {
  ...
  chrome.sendFrameMessage({
    type: "atelier:whiteboardRelay",
    diagramIndex: 0,
    message: { type: "atelier-whiteboard:save", scene: { elements: [{ id: "forged" }] } },
  });
  await flushPromises();
...
test("whiteboard fullscreen waits for the authenticated inline frame to flush", async () => {
  ...
  assert.equal(init.type, "atelier-whiteboard:init");
  assert.equal(init.channelId, "inline-channel");

  chrome.sendInlineWhiteboardMessage(inline, {
    type: "atelier-whiteboard:maximize",
    diagramIndex: 0,
    channelId: "inline-channel",
  });

  const prepare = inline.posted.at(-1);
  assert.equal(prepare.type, "atelier-whiteboard:prepareTeardown");
  assert.equal(
    chrome.postedToFrame.some((message) => message.type === "atelier:suspendWhiteboard"),
    false,
  );

  chrome.sendInlineWhiteboardMessage(inline, {
    type: "atelier-whiteboard:teardownReady",
    diagramIndex: 0,
    channelId: "inline-channel",
...
test("whiteboard close waits for the authenticated overlay frame to flush", async () => {
  ...
  chrome.sendWhiteboardMessage({ type: "atelier-whiteboard:ready", diagramIndex: 0, channelToken: "overlay-channel" });
  await flushPromises();
  await flushPromises();

  chrome.element("whiteboardClose").click();
  const closePrepare = chrome.postedToWhiteboard.at(-1);
  assert.equal(closePrepare.type, "atelier-whiteboard:prepareTeardown");
  ...
  assert.equal(chrome.postedToFrame.at(-1).type, "atelier:resumeWhiteboard");
});
...
test("whiteboard fullscreen close accepts the resumed inline frame", async () => {
  ...
  assert.equal(resumed.posted.at(-1).type, "atelier-whiteboard:init");
...
test("server restart flushes an authenticated inline whiteboard before reloading", async () => {
  ...
  assert.equal(flush.type, "atelier-whiteboard:flush");
  assert.equal(chrome.reloadCount(), 0);

  chrome.sendInlineWhiteboardMessage(inline, {
    type: "atelier-whiteboard:flushComplete",
    diagramIndex: 0,
    channelId: "inline-channel",
...
test("server restart bounds the wait for a whiteboard flush", async () => {
  ...
  assert.equal(inline.posted.at(-1).type, "atelier-whiteboard:flush");
...
test("whiteboard close stays responsive while overlay initialization is pending", async () => {
  ...
  assert.equal(chrome.postedToFrame.at(-1).type, "atelier:resumeWhiteboard");
  assert.equal(
    chrome.postedToWhiteboard.some((message) => message.type === "atelier-whiteboard:prepareTeardown"),
    false,
  );
...
test("chrome uploads captured attachment bytes and reports the server id to the card", async () => {
  ...
  assert.equal(result.type, "atelier:attachmentResult");
...
test("a queued prompt over the thumbnail limit shows the hidden images as a +N badge (W-A)", async () => {
  // ATELIER_AXI_MAX_ATTACHMENTS_PER_PROMPT is configurable, so a prompt can legitimately
...
test("the chrome never honors an attachment delete driven by the artifact iframe (E2)", async () => {
  ...
  chrome.sendFrameMessage({ type: "atelier:removeAttachment", id: "a".repeat(64) + ".png" });
  await flushPromises();
...
test("the chrome bounds concurrent in-flight uploads (D8)", async () => {
  ...
  const refused = chrome.postedToFrame.filter(
    (m) =>
      m.type === "atelier:attachmentResult" &&
      m.ok === false &&
