...
async function withTempDir(run) {
  const dir = await mkdtemp(path.join(os.tmpdir(), "atelier-attach-"));
  try {
    await run(dir);
  } finally {
...
test("resolveAttachmentConfig reads ATELIER_AXI_* limits with sane fallbacks", () => {
  ...
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENT_DISK_MB: "off" }).maxObjects, null);

  const custom = resolveAttachmentConfig({
    ATELIER_AXI_MAX_ATTACHMENT_BYTES: "2048",
    ATELIER_AXI_MAX_ATTACHMENTS_PER_PROMPT: "2",
    ATELIER_AXI_ATTACHMENT_TTL_MS: "off",
    ATELIER_AXI_MAX_ATTACHMENT_DISK_MB: "50",
  });
  ...
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENT_DISK_MB: "off" }).maxDiskBytes, null);
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENT_DISK_MB: "0" }).maxDiskBytes, null);

  // Non-positive / unparseable values fall back rather than throwing.
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENT_BYTES: "-1" }).maxBytes, 10 * 1024 * 1024);
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_ATTACHMENT_TTL_MS: "0" }).ttlMs, null);
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENT_DISK_MB: "-5" }).maxDiskBytes, 512 * 1024 * 1024);
});
...
test("a fractional limit that floors below 1 falls back instead of disabling the cap (W5)", () => {
  ...
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENTS_PER_PROMPT: "0.5" }).maxPerPrompt, 4);
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENT_BYTES: "0.9" }).maxBytes, 10 * 1024 * 1024);
  assert.equal(
    resolveAttachmentConfig({ ATELIER_AXI_MAX_PROMPT_ATTACHMENT_BYTES: "0.25" }).maxPromptBytes,
    25 * 1024 * 1024,
  );
  // A fractional value that still floors to >= 1 is honored, floored.
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENTS_PER_PROMPT: "2.7" }).maxPerPrompt, 2);
  ...
  assert.equal(
    resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENT_DISK_MB: "0.0000001" }).maxDiskBytes,
    512 * 1024 * 1024,
  );
  assert.equal(resolveAttachmentConfig({ ATELIER_AXI_MAX_ATTACHMENT_DISK_MB: "0.5" }).maxDiskBytes, 512 * 1024);
});
