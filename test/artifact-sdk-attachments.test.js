...
test("the SDK bundle uploads captured images through the chrome", () => {
  ...
  assert.match(sdk, /postArtifactMessage\("atelier:uploadAttachment", \{/);
  assert.doesNotMatch(sdk, /parent\.postMessage\(\s*\{\s*type: "atelier:uploadAttachment"/);
...
test("the SDK bundle applies upload results and offers a retry", () => {
  assert.match(sdk, /atelier:attachmentResult/);
...
test("the SDK bundle renders chips with a thumbnail, name, and status", () => {
  assert.match(sdk, /atelier-attachment-thumb/);
  assert.match(sdk, /atelier-attachment-name/);
...
test("the SDK bundle renders a visible, titled remove control on each chip", () => {
  ...
  assert.match(sdk, /atelier-attachment-remove/);
});
...
test("the count-cap notice reads as an error, not as the passive keyboard hint", () => {
  ...
  assert.match(sdk, /atelier-hint-alert/);
  assert.match(sdk, /\.atelier-hint-alert\{[^}]*color:#ff9d7a/);
  assert.match(sdk, /attachNotice\.classList\.add\("atelier-hint-alert"\)/);
  // Clearing the notice restores the neutral hint instead of leaving stale red text.
  assert.match(sdk, /attachNotice\.classList\.remove\("atelier-hint-alert"\)/);
});
