...
test("the configured diagnostic viewport set falls back to every class", () => {
  ...
  assert.deepEqual(resolveDiagnosticViewportClasses({ ATELIER_AXI_DIAGNOSTIC_VIEWPORTS: "desktop, mobile" }), [
    "desktop",
    "mobile",
  ]);
  assert.deepEqual(resolveDiagnosticViewportClasses({ ATELIER_AXI_DIAGNOSTIC_VIEWPORTS: "nonsense" }), [
    "mobile",
    "compact",
    "desktop",
  ]);
