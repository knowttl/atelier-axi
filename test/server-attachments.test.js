...
async function withSession(run, { env } = {}) {
  const dir = await mkdtemp(path.join(tmpdir(), "atelier-attach-srv-"));
...
test("upload rejects bytes over the configured per-image cap with 413", async () => {
  await withSession(
    ...
    { env: { ATELIER_AXI_MAX_ATTACHMENT_BYTES: "8" } },
  );
});
...
test("export still works and leaks no attachment data when a prompt references an image", async () => {
  await withSession(async ({ base, key }) => {
    ...
    assert.doesNotMatch(html, /atelier:uploadAttachment/);
  });
});

test("the server sweeps an expired, unreferenced attachment at startup", async () => {
  const dir = await mkdtemp(path.join(tmpdir(), "atelier-attach-sweep-"));
  ...
  const saved = process.env.ATELIER_AXI_ATTACHMENT_TTL_MS;
  process.env.ATELIER_AXI_ATTACHMENT_TTL_MS = "1000";
  ...
  try {
    ...
    await server.close();
    if (saved === undefined) delete process.env.ATELIER_AXI_ATTACHMENT_TTL_MS;
    else process.env.ATELIER_AXI_ATTACHMENT_TTL_MS = saved;
    await rm(dir, { recursive: true, force: true });
  }
});
