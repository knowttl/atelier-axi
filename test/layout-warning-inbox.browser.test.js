...
// audit returned straight out of `atelier-axi poll` with no user action, so several individually
...
const runBrowserE2e = process.env.ATELIER_AXI_BROWSER_E2E === "1";
...
test(
  ...
  async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "atelier-warning-inbox-"));
    const port = await freePort();
    const atelierEnv = {
      ATELIER_AXI_PORT: String(port),
      ATELIER_AXI_STATE_DIR: path.join(temp, "state"),
      ATELIER_AXI_NO_OPEN: "1",
      ATELIER_AXI_TELEMETRY: "0",
      ATELIER_AXI_HOST: "127.0.0.1",
      ATELIER_AXI_LINK_HOST: "127.0.0.1",
    };
    const chromeEnv = {
      CHROME_DEVTOOLS_AXI_SESSION: `atelier-warning-inbox-${process.pid}`,
      CHROME_DEVTOOLS_AXI_USER_DATA_DIR: path.join(temp, "chrome"),
    };

    function openArtifact(file, args = []) {
      const output = run(process.execPath, ["bin/atelier-axi.js", file, "--no-open", ...args], atelierEnv);
    ...
    function poll(file, timeoutMs) {
      return run(
        process.execPath,
        ["bin/atelier-axi.js", "poll", file, "--timeout-ms", String(timeoutMs)],
        atelierEnv,
        timeoutMs + 45_000,
      );
    }
    ...
    function inbox() {
      return evaluate(
        "JSON.stringify({" +
          ...
          " loads: window.__atelierArtifactLoads," +
    ...
    function instrumentArtifactLoads() {
      run(
        ...
        [
          "eval",
          '() => { window.__atelierArtifactLoads = 0; if (window.__atelierArtifactLoadsWired) return "reset"; window.__atelierArtifactLoadsWired = true; document.getElementById("artifact").addEventListener("load", () => { window.__atelierArtifactLoads += 1; }); return "wired"; }',
        ],
        chromeEnv,
      );
    }
...
test(
  "a live reload preserves the review context Atelier owns",
  { skip: !runBrowserE2e, timeout: 240_000 },
  async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "atelier-review-context-"));
    const port = await freePort();
    const atelierEnv = {
      ATELIER_AXI_PORT: String(port),
      ATELIER_AXI_STATE_DIR: path.join(temp, "state"),
    ...
    const chromeEnv = {
      CHROME_DEVTOOLS_AXI_SESSION: `atelier-review-context-${process.pid}`,
      CHROME_DEVTOOLS_AXI_USER_DATA_DIR: path.join(temp, "chrome"),
    };
    ...
    try {
      ...
      const output = run(process.execPath, ["bin/atelier-axi.js", artifact, "--no-open"], atelierEnv);
      const url = output.match(/url:\s*"([^"]+)"/)?.[1];
      assert.ok(url, output);
      run("chrome-devtools-axi", ["emulate", "--viewport", "1440x1000x1"], chromeEnv);
      run("chrome-devtools-axi", ["open", url], chromeEnv);
      ...
      // Atelier-owned answers: a radio and a checkbox inside a data-atelier-question scope.
      click(/radio " Pro"/);
      ...
      assert.match(after, /radio " Pro" checked/, "a Atelier-owned answer survives the reload");
