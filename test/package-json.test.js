...
test("atelier-design agent skill is marked internal for skills CLI discovery", async () => {
  const skillMd = await readFile(new URL("../.agents/skills/atelier-design/SKILL.md", import.meta.url), "utf8");
  const frontmatter = skillMd.slice(4, skillMd.indexOf("\n---\n", 4));

  assert.match(frontmatter, /^name: atelier-design$/m);
  assert.match(frontmatter, /^metadata:\n {2}internal: true$/m);
});

test("public atelier skill is not marked internal", async () => {
  const skillMd = await readFile(new URL("../skills/atelier/SKILL.md", import.meta.url), "utf8");
  const frontmatter = skillMd.slice(4, skillMd.indexOf("\n---\n", 4));
