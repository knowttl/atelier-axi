...
test("telemetry uses env values before build-time defaults", () => {
  const config = resolveTelemetryConfig({
    env: {
      ATELIER_AXI_UMAMI_HOST: " https://env.example ",
      ATELIER_AXI_UMAMI_WEBSITE_ID: " env-id ",
    },
    buildHost: "https://build.example",
    buildWebsiteID: "build-id",
  });
...
test("telemetry sends anonymous Umami event payloads", async () => {
  ...
  assert.match(requests[0].headers["User-Agent"], /^atelier-axi\/1\.2\.3 telemetry$/);
  assert.deepEqual(requests[0].body.payload.website, "site-1");
  assert.deepEqual(requests[0].body.payload.hostname, "cli");
  assert.deepEqual(requests[0].body.payload.title, "Atelier Editor CLI");
  assert.deepEqual(requests[0].body.payload.url, "app://atelier-axi/command");
