...
test(
  ...
  async () => {
    const temp = await mkdtemp(path.join(tmpdir(), "atelier-layout-browser-"));
    ...
    const chromeEnv = {
      CHROME_DEVTOOLS_AXI_SESSION: `atelier-layout-${process.pid}`,
      CHROME_DEVTOOLS_AXI_USER_DATA_DIR: path.join(temp, "chrome"),
    };

    function openArtifact(file) {
      const output = run(process.execPath, ["bin/atelier-axi.js", file, "--no-open"], atelierEnv);
    ...
    function audit(name, viewport, settleMs, expectedCount) {
      ...
      let poll = run(process.execPath, ["bin/atelier-axi.js", "poll", file, "--timeout-ms", pollTimeout], atelierEnv);
      const expectedWarnings = new RegExp(`layout_warnings\\[${expectedCount}\\]`);
      if (expectedCount > 0 && !expectedWarnings.test(poll)) {
        run("chrome-devtools-axi", ["open", url], chromeEnv);
        run("chrome-devtools-axi", ["wait", String(settleMs)], chromeEnv, settleMs + 45_000);
        poll = run(process.execPath, ["bin/atelier-axi.js", "poll", file, "--timeout-ms", pollTimeout], atelierEnv);
      }
    ...
    try {
      ...
      assert.match(
        run(process.execPath, ["bin/atelier-axi.js", "poll", revalidationFile, "--timeout-ms", "8000"], atelierEnv),
        /layout_warnings\[3\]/,
      );
      await writeFile(
        revalidationFile,
