...
test(
  ...
  async () => {
    ...
    async function audit(name, viewport, settleMs, expectedCount) {
      ...
      const poll = run(process.execPath, ["bin/atelier-axi.js", "poll", file, "--timeout-ms", "600"], atelierEnv);
    ...
    try {
      ...
      assert.match(
        run(process.execPath, ["bin/atelier-axi.js", "poll", revalidationFile, "--timeout-ms", "600"], atelierEnv),
        /status:\s*waiting/,
      );
