...
// sent atelier:uploadAttachment via a raw parent.postMessage with no
...
(function () {
  var PNG_B64 = "${PNG_B64}";
  function shadow() {
    var host = document.querySelector(".atelier-annotation-root");
  ...
  async function drive() {
    try {
      ...
      for (var i = 0; i < 100 && !card; i += 1) {
        await sleep(100);
        var root = shadow();
        card = root ? root.querySelector(".atelier-annotation-card") : null;
      }
      ...
      for (var k = 0; k < 150; k += 1) {
        await sleep(100);
        var chip = card.querySelector(".atelier-attachment-chip");
        var error = chip ? chip.querySelector(".atelier-attachment-status-error") : null;
        ...
        if (chip && !chip.querySelector(".atelier-attachment-status")) {
          mark("ready");
          textarea.value = "e2e: attachment round trip";
          card.querySelector(".atelier-send").click();
          mark("queued");
          return;
        }
      }
...
test(
  ...
  async () => {
    ...
    const chromeEnv = {
      CHROME_DEVTOOLS_AXI_SESSION: `atelier-attach-e2e-${process.pid}`,
