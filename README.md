...
- **Agent presence** - The browser shows when no agent is listening, shows a **Working...** bubble while the agent is applying delivered feedback, and keeps queued feedback and proven severe layout failures for the next successful `atelier-axi poll` send even across reloads.
