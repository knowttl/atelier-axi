...
- **Agent presence** - The browser shows when no agent is listening, keeps queued feedback and proven severe layout failures for the next successful `atelier-axi poll` send even across reloads, and only blocks human sends while the agent is working on delivered feedback.
