...
`atelier-axi share` publishes to [ht-ml.app](https://ht-ml.app) by default. Set
`ATELIER_AXI_HTML_APP_API_URL` to point `share` at a backend you control instead.
...
| Env var                       | Purpose                                                                                                |
| ----------------------------- | ------------------------------------------------------------------------------------------------------ |
| `ATELIER_AXI_HTML_APP_API_URL` | Base URL of your share backend. Defaults to `https://api.ht-ml.app`; trailing slashes are stripped.    |
| `ATELIER_AXI_HTML_APP_TOKEN`   | Optional bearer token, sent as `Authorization: Bearer <token>`. Also settable per call with `--token`. |
...
```
POST {ATELIER_AXI_HTML_APP_API_URL}/v1/sites
