...
- `atelier-axi` is a maintained fork of [`kunchenguid/lavish-axi`](https://github.com/kunchenguid/lavish-axi) carrying a permanent `lavish` -> `atelier` rebrand. Upstream syncs are the one exception to the `no-mistakes` rule above. GitHub's behind-upstream counter tracks ancestry rather than content, and a sync is not finished until `behind_by` reads `0`. See `docs/upstream-sync.md` for the required procedure, including which mode applies and which recorded tip to target.
