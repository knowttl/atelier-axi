...
- `atelier-axi` is a maintained fork of [`kunchenguid/lavish-axi`](https://github.com/kunchenguid/lavish-axi) carrying a permanent `lavish` -> `atelier` rebrand. Upstream syncs follow their own maintainer procedure (throwaway `sync-upstream-<date>` branch, plain merge commit, admin-merge past the expected-red `Require no-mistakes` check) and are the one exception to the rule above - see `docs/upstream-sync.md`. Every sync must END with an `ours` reconciliation merge of `upstream/main` (empty first-parent diff), because GitHub's behind-upstream counter tracks ancestry rather than content and ported or skipped commits never clear it on their own.
