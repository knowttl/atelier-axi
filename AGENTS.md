...
5. The injected SDK also runs a render-time layout audit after `document.fonts.ready` and a brief `ResizeObserver` settle window.
   ...
   The gate re-arms on hot reload and reveals after the next completed audit. Unresolved issues remain signaled by the top-bar inbox, with a one-time banner until the reviewer acknowledges it; **Show anyway** reveals immediately, and a bounded safety timeout also reveals because a delayed or unavailable audit is uncertainty, not a defect.
