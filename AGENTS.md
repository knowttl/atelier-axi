...
- `SessionStore` re-reads and re-writes the entire `state.json` on every operation. There's no in-memory cache, but every method funnels its read-modify-write through `serialize()` (a promise-chain lock on `this.tail`), so operations never interleave. That lock is load-bearing: without it a batch send (`queuePrompts`) racing a concurrent writer - the fresh-session layout audit (`recordLayoutWarnings`) or an active poll's drain (`takeFeedback`) - dropped queued prompts (later writer clobbers the earlier read) and interleaved `writeFile` calls corrupted the file. Keep any new store method inside `serialize()`.
