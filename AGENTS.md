...
Beyond that generated `atelier` skill, the package ships two hand-authored prose skills (added to the `files` allowlist: `skills/atelier-plan`, `skills/atelier-implement`) that drive the feature-planning pipeline surfaced by the enriched `plan` playbook: `atelier-plan` runs the visual planning arc - surface every open question, edge case, and candidate approach as an annotatable review surface, converge on an approved direction, then write durable `spec.md`/`plan.md` (large route) or `plan.md` only (small route) under `docs/atelier/<YYYY-MM-DD>-<type>-<topic>/` plus beads records - and `atelier-implement` executes a finished `plan.md` task-by-task with a fresh subagent per task, TDD, review between tasks, and a final whole-branch review. These two are edited directly (not regenerated by `build:skill`), stay self-contained (they depend on no other skills except `atelier-plan` optionally invoking `atelier-implement` on user opt-in), and degrade gracefully when `atelier-axi` or `bd` is absent.
