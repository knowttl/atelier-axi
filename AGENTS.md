...
8. `atelier-axi poll <file.html>` (`pollCommand`) hits `GET /api/poll`.
   ...
   `takeFeedback` is a destructive read with no addressing, so **a session may have only one poll**: whichever poll drains a batch holds the only copy, and a leftover poll (a backgrounded or abandoned `atelier-axi poll` whose output nobody reads) silently swallows the user's feedback while the live agent's poll answers `waiting`.
   The server therefore keeps a `waitingPolls` registry and retires every other waiting poll for a key - with `{ status: "superseded" }` and **without draining** - both when a new poll attaches and when a delivery completes; the second point matters because the window an agent spends applying feedback is exactly when the reviewer sends the next batch.
   `createPollOutput` turns `superseded` into a `POLL_SUPERSEDED` `AxiError` so losing the session is loud, and `POLL_WAKE_PATH_RULES` states the one-poll-per-artifact rule. Never make delivery drain for a poll that is not the session's current owner.
