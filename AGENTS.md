...
8. `atelier-axi poll <file.html>` (`pollCommand`) hits `GET /api/poll`.
   ...
   Every delivered poll response ends with one machine-checkable trailer line, `prompts_delivered=<N> batch_file=<path>` (`formatPollDeliveryTrailer`/`renderPollDelivery` in `src/cli.js`), and `pollCommand` writes the whole response to that file (`pollBatchFile` in `src/paths.js`) before rendering.
