...
5. The injected SDK also runs a render-time layout audit after `document.fonts.ready` and a brief `ResizeObserver` settle window.
   ...
   The artifact posts `atelier:layoutWarnings` to the chrome with structured `layout_warnings` entries containing `selector`, `kind`, `overflowPx`, `viewportWidth`, and `severity` (always `error` for the findings that survive to the chrome).
