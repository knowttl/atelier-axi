...
Top-level `--help` returns the same static guidance without dynamic sessions, `atelier-axi playbook [playbook_id]` exposes focused artifact guidance, and `atelier-axi design` prints a content-to-playbook router, copy-pasteable Tailwind/DaisyUI CDN URLs, Mermaid diagram tooling, an optional layout safety CSS snippet, and the DaisyUI component reference as a fallback after user-specified and subject-project design sources come up empty.
