...
5. The injected SDK also runs a render-time layout audit after `document.fonts.ready` and a brief `ResizeObserver` settle window.
   ...
   The artifact posts `atelier:layoutWarnings` to the chrome with structured `layout_warnings` entries containing `selector`, `kind`, `overflowPx`, `viewportWidth`, and `severity`.
...
- `SessionStore` re-reads and re-writes the entire `state.json` on every operation. There's no in-memory cache, but every method funnels its read-modify-write through `serialize()`, which combines an instance promise chain with an advisory lock shared by every process using that state file. Writes atomically rename a same-directory temporary file over `state.json`, so CLI readers never observe a partial write. The locks are load-bearing: without them a batch send (`queuePrompts`) racing a concurrent writer - the fresh-session layout audit (`recordLayoutWarnings`) or an active poll's drain (`takeFeedback`) - dropped queued prompts when a later writer clobbered the earlier read. Keep any new store method inside `serialize()`.
